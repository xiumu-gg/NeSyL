...
def eval_for_tail(eval_data, model, device, data, descending, raoit=0):
    ...
    ranks_left = []
    ranks_right = []
    ent_rel_multi_t = data['as_tail']
    ent_rel_multi_h = data['as_head']
    ...
    for batch_idx, batch_data in enumerate(tqdm(eval_data)):
        ...
        eval_ch = batch_data[1][0]
        eval_ct = batch_data[1][1]
        eval_cr = batch_data[1][2]
        _, pred = model(eval_h, eval_r, eval_ch)  # evaluate corruptions by replacing the object, i.e. tail entity
        # _, pred1 = model(eval_t, eval_r, inverse=True)
        _, pred1 = model(eval_t, eval_r, eval_ct)
